...
router.post('/mods/:id/approve', (req, res) => {
  ...
  logAction('Telegram-бот', 'approve_mod', mod.name);
...
router.post('/mods/:id/reject', (req, res) => {
  ...
  db.prepare(`UPDATE mods SET status = 'rejected', moderation_note = ? WHERE id = ?`).run(note, mod.id);
  logAction('Telegram-бот', 'reject_mod', mod.name, note);
  res.json({ ok: true, name: mod.name });
});
...
router.post('/mod-versions/:id/approve', (req, res) => {
  ...
  logAction('Telegram-бот', 'approve_version', `${version.mod_name} — ${version.version_label}`);
...
router.post('/mod-versions/:id/reject', (req, res) => {
  ...
  logAction('Telegram-бот', 'reject_version', `${version.mod_name} — ${version.version_label}`);
  res.json({ ok: true, name: version.mod_name, label: version.version_label });
});
