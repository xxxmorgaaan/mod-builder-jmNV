...
const { toCsv, humanSize, revealControlCode, moscowDateStr } = require('../src/helpers');
...
router.post('/admin/mod-versions/:id/approve', (req, res) => {
  const version = db.prepare('SELECT mod_id FROM mod_versions WHERE id = ?').get(req.params.id);
  db.prepare(`UPDATE mod_versions SET status = 'approved' WHERE id = ?`).run(req.params.id);
  if (version) db.prepare(`UPDATE mods SET updated_at = datetime('now') WHERE id = ?`).run(version.mod_id);
  res.redirect('back');
});
...
router.post('/admin/ban-ip', (req, res) => {
  ...
  if (ip) db.prepare('INSERT INTO banned_ips (ip, reason) VALUES (?, ?) ON CONFLICT(ip) DO UPDATE SET reason = excluded.reason').run(ip, reason);
  res.redirect('/admin/users');
});
router.post('/admin/unban-ip', (req, res) => {
  db.prepare('DELETE FROM banned_ips WHERE ip = ?').run(req.body.ip || '');
  res.redirect('/admin/users');
});
